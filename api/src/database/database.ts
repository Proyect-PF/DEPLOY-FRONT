...
const db_deploy = "postgresql://postgres:ar1cxwVG4hcZhPtpSunm@containers-us-west-55.railway.app:6434/railway"
...
Object.keys(db).forEach((modelName) => {
  if (db[modelName].associate) {
    ...
    for (const association in db[modelName].associations) {
      console.log(
        ` MODELO: ${association} : ASOCIACION ${db[modelName].associations[association].associationType}`
      );
    }
  }
});
